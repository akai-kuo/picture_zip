...
function updateUI() {
  ...
  if (doneCount > 0) {
    ...
    const totalOriginal = state.files
      .filter((f) => f.status === "done")
      .reduce((s, f) => s + f.originalSize, 0);
    const totalCompressed = state.files
      .filter((f) => f.status === "done")
      .reduce((s, f) => s + (f.resultSize || 0), 0);
...
function createImageItemEl(item) {
  ...
  const progressHtml =
    item.status === "processing"
      ...
      : item.status === "done"
        ? `<div class="progress-wrap"><div class="progress-bar-bg"><div class="progress-bar-fill done" style="width:100%"></div></div></div>`
        : "";
...
async function compressImage(item) {
  ...
  if (maxW) {
    fd.append("maxWidth", maxW);
  }

  const res = await fetch(`${API_BASE_URL}/api/images/process`, {
    method: "POST",
    body: fd,
  });
  ...
  try {
    ...
    throw new Error(`伺服器回傳非 JSON 資料（HTTP ${res.status}）`);
  }

  if (!res.ok || json.success !== true) {
    throw new Error(json.error?.message || `伺服器處理失敗（HTTP ${res.status}）`);
  }
...
async function startCompression() {
  ...
  for (const item of pending) {
    ...
    try {
      ...
      item.downloadUrl = `${API_BASE_URL}${result.downloadUrl}`; // 後端提供的下載路徑
...
async function downloadItem(id) {
  ...
  const ext = item.outputFormat === "jpeg" ? "jpg" : item.outputFormat || "webp";
  ...
  try {
    ...
    showToast(`${item.name}：下載失敗`, "error");
  }
}
...
uploadZone.addEventListener("click", (e) => {
  if (
    e.target === uploadZone ||
    e.target.closest(".upload-icon-wrap") ||
